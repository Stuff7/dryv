...
impl Frame {
  ...
  pub fn intra4x4_prediction(&mut self, slice: &mut Slice, luma4x4_blk_idx: usize, is_luma: bool) {
    ...
    let x_o = inverse_raster_scan(luma4x4_blk_idx as isize / 4, 8, 8, 16, 0) + inverse_raster_scan(luma4x4_blk_idx as isize % 4, 4, 4, 8, 0);
    let y_o = inverse_raster_scan(luma4x4_blk_idx as isize / 4, 8, 8, 16, 1) + inverse_raster_scan(luma4x4_blk_idx as isize % 4, 4, 4, 8, 1);
    ...
    for i in 0..13 {
      ...
      let mb_n = pos_a.map(|pos| slice.mb_nb_p(pos, 0)).unwrap_or_else(|| Macroblock::unavailable(0));
        ...
        || (x > 3) && (luma4x4_blk_idx == 3 || luma4x4_blk_idx == 11)
      {
        *samples.p(x, y) = -1;
        ...
        let x_m = inverse_raster_scan(mbaddr_n as isize, 16, 16, slice.pic_width_in_samples_l as isize, 0);
        let y_m = inverse_raster_scan(mbaddr_n as isize, 16, 16, slice.pic_width_in_samples_l as isize, 1);

        *samples.p(x, y) = self.luma_data[(x_m + x_w) as usize][(y_m + y_w) as usize] as isize;
      }
    }

    if *samples.p(4, -1) < 0 && *samples.p(5, -1) < 0 && *samples.p(6, -1) < 0 && *samples.p(7, -1) < 0 && *samples.p(3, -1) >= 0 {
    ...
    if intra4x4_pred_mode == INTRA_4X4_VERTICAL {
      if *samples.p(0, -1) >= 0 && *samples.p(1, -1) >= 0 && *samples.p(2, -1) >= 0 && *samples.p(3, -1) >= 0 {
        for y in 0..4 {
          for x in 0..4 {
      ...
      if *samples.p(-1, 0) >= 0 && *samples.p(-1, 1) >= 0 && *samples.p(-1, 2) >= 0 && *samples.p(-1, 3) >= 0 {
        for y in 0..4 {
          for x in 0..4 {
        ...
        && *samples.p(-1, 3) >= 0
      {
        val = (*samples.p(0, -1)
          ...
          + 4)
          >> 3;
      } else if (*samples.p(0, -1) < 0 || *samples.p(1, -1) < 0 || *samples.p(2, -1) < 0 || *samples.p(3, -1) < 0)
        && (*samples.p(-1, 0) >= 0 && *samples.p(-1, 1) >= 0 && *samples.p(-1, 2) >= 0 && *samples.p(-1, 3) >= 0)
      {
        val = (*samples.p(-1, 0) + *samples.p(-1, 1) + *samples.p(-1, 2) + *samples.p(-1, 3) + 2) >> 2;
      } else if (*samples.p(0, -1) >= 0 && *samples.p(1, -1) >= 0 && *samples.p(2, -1) >= 0 && *samples.p(3, -1) >= 0)
        && (*samples.p(-1, 0) < 0 || *samples.p(-1, 1) < 0 || *samples.p(-1, 2) < 0 || *samples.p(-1, 3) < 0)
      {
        val = (*samples.p(0, -1) + *samples.p(1, -1) + *samples.p(2, -1) + *samples.p(3, -1) + 2) >> 2;
      } else {
        ...
        && *samples.p(7, -1) >= 0
      {
        for y in 0..4isize {
          for x in 0..4isize {
            if x == 3 && y == 3 {
              slice.mb_mut().luma_pred_samples[luma4x4_blk_idx][x as usize][y as usize] = (*samples.p(6, -1) + 3 * *samples.p(7, -1) + 2) >> 2;
            } else {
              slice.mb_mut().luma_pred_samples[luma4x4_blk_idx][x as usize][y as usize] =
                (*samples.p(x + y, -1) + 2 * *samples.p(x + y + 1, -1) + *samples.p(x + y + 2, -1) + 2) >> 2;
            }
          }
        }
      }
        ...
        for y in 0..=3isize {
          for x in 0..=3isize {
            match x.cmp(&y) {
              Ordering::Greater => {
                slice.mb_mut().luma_pred_samples[luma4x4_blk_idx][x as usize][y as usize] =
                  (*samples.p(x - y - 2, -1) + 2 * *samples.p(x - y - 1, -1) + *samples.p(x - y, -1) + 2) >> 2;
              }
              Ordering::Less => {
                slice.mb_mut().luma_pred_samples[luma4x4_blk_idx][x as usize][y as usize] =
                  (*samples.p(-1, y - x - 2) + 2 * *samples.p(-1, y - x - 1) + *samples.p(-1, y - x) + 2) >> 2;
              }
              Ordering::Equal => {
                slice.mb_mut().luma_pred_samples[luma4x4_blk_idx][x as usize][y as usize] =
        ...
        for y in 0..=3isize {
          for x in 0..=3isize {
            ...
            if z_vr == 0 || z_vr == 2 || z_vr == 4 || z_vr == 6 {
              ...
              slice.mb_mut().luma_pred_samples[luma4x4_blk_idx][x as usize][y as usize] =
                (*samples.p(x - (y >> 1) - 2, -1) + 2 * *samples.p(x - (y >> 1) - 1, -1) + *samples.p(x - (y >> 1), -1) + 2) >> 2;
            } else if z_vr == -1 {
              slice.mb_mut().luma_pred_samples[luma4x4_blk_idx][x as usize][y as usize] =
                (*samples.p(-1, 0) + 2 * *samples.p(-1, -1) + *samples.p(0, -1) + 2) >> 2;
            } else {
              slice.mb_mut().luma_pred_samples[luma4x4_blk_idx][x as usize][y as usize] =
                (*samples.p(-1, y - 1) + 2 * *samples.p(-1, y - 2) + *samples.p(-1, y - 3) + 2) >> 2;
            }
          }
        }
      }
        ...
        for y in 0..=3isize {
          for x in 0..=3isize {
            ...
            if z_hd == 0 || z_hd == 2 || z_hd == 4 || z_hd == 6 {
              ...
              slice.mb_mut().luma_pred_samples[luma4x4_blk_idx][x as usize][y as usize] =
                (*samples.p(-1, y - (x >> 1) - 2) + 2 * *samples.p(-1, y - (x >> 1) - 1) + *samples.p(-1, y - (x >> 1)) + 2) >> 2;
            } else if z_hd == -1 {
              slice.mb_mut().luma_pred_samples[luma4x4_blk_idx][x as usize][y as usize] =
                (*samples.p(-1, 0) + 2 * *samples.p(-1, -1) + *samples.p(0, -1) + 2) >> 2;
            } else {
              slice.mb_mut().luma_pred_samples[luma4x4_blk_idx][x as usize][y as usize] =
                (*samples.p(x - 1, -1) + 2 * *samples.p(x - 2, -1) + *samples.p(x - 3, -1) + 2) >> 2;
            }
          }
        }
      }
        ...
        for y in 0..=3isize {
          for x in 0..=3isize {
            if y == 0 || y == 2 {
              ...
              slice.mb_mut().luma_pred_samples[luma4x4_blk_idx][x as usize][y as usize] =
                (*samples.p(x + (y >> 1), -1) + 2 * *samples.p(x + (y >> 1) + 1, -1) + *samples.p(x + (y >> 1) + 2, -1) + 2) >> 2;
            }
          }
        }
      }
      ...
      for y in 0..=3isize {
        for x in 0..=3isize {
          ...
          if z_hu == 0 || z_hu == 2 || z_hu == 4 {
            ...
            slice.mb_mut().luma_pred_samples[luma4x4_blk_idx][x as usize][y as usize] =
              (*samples.p(-1, y + (x >> 1)) + 2 * *samples.p(-1, y + (x >> 1) + 1) + *samples.p(-1, y + (x >> 1) + 2) + 2) >> 2;
          } else if z_hu == 5 {
            slice.mb_mut().luma_pred_samples[luma4x4_blk_idx][x as usize][y as usize] = (*samples.p(-1, 2) + 3 * *samples.p(-1, 3) + 2) >> 2;
          } else {
            slice.mb_mut().luma_pred_samples[luma4x4_blk_idx][x as usize][y as usize] = *samples.p(-1, 3);
          }
        }
      }
    }
  ...
  pub fn intra4x4_pred_mode(&mut self, slice: &mut Slice, luma4x4_block_idx: usize, is_luma: bool) {
    ...
    let x = inverse_raster_scan(luma4x4_block_idx as isize / 4, 8, 8, 16, 0) + inverse_raster_scan(luma4x4_block_idx as isize % 4, 4, 4, 8, 0);
    let y = inverse_raster_scan(luma4x4_block_idx as isize / 4, 8, 8, 16, 1) + inverse_raster_scan(luma4x4_block_idx as isize % 4, 4, 4, 8, 1);
    ...
    if dc_pred_mode_predicted_flag || (!mb_a.mb_type.mode().is_intra_4x4() && !mb_a.mb_type.mode().is_intra_8x8()) {
    ...
    if dc_pred_mode_predicted_flag || (!mb_b.mb_type.mode().is_intra_4x4() && !mb_b.mb_type.mode().is_intra_8x8()) {
    ...
    } else if (slice.mb().rem_intra4x4_pred_mode[luma4x4_block_idx] as isize) < pred_intra4x4_pred_mode {
      slice.mb_mut().intra4x4_pred_mode[luma4x4_block_idx] = slice.mb().rem_intra4x4_pred_mode[luma4x4_block_idx] as isize;
    } else {
      slice.mb_mut().intra4x4_pred_mode[luma4x4_block_idx] = slice.mb().rem_intra4x4_pred_mode[luma4x4_block_idx] as isize + 1;
    }
  }
}
